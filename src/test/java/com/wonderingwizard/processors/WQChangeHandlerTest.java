...
/**
 * Tests for WQ Change event handling.
 */
@DisplayName("WQ Change Handler")
class WQChangeHandlerTest {
    ...
    private static final long WI_ID_1 = 8001L;
    private static final long WI_ID_2 = 8002L;
    ...
    void setUp() {
        ...
        scheduleRunner.registerSubProcessor(new WQChangeHandler());
    ...
    private WorkInstructionEvent wqChangeEvent(long workInstructionId) {
        return new WorkInstructionEvent(
                EventType.WQ_CHANGE, workInstructionId, 1L, "QC01", "Planned",
    ...
    class Cancellation {
        ...
        void cancelBeforeHandover() {
            ...
            List<SideEffect> effects = engine.processEvent(wqChangeEvent(WI_ID_1));
            ...
            assertTrue(completedEffects.stream().allMatch(ac ->
                    ac.reason() == CompletionReason.WQ_CHANGE),
                    "All completed actions should have WQ_CHANGE reason");
        }
        ...
        void cancelTwinContainerActions() {
            engine.processEvent(workingTruck("TT01"));
            engine.processEvent(twinScheduleWithTTWorkflow(1L));
            engine.processEvent(new TimeEvent(now));
        ...
        void completionReasonOnAction() {
            ...
            engine.processEvent(wqChangeEvent(WI_ID_1));
            ...
            for (Action action : schedule.takts().getFirst().actions()) {
                ...
                if (current != null && current.completionReason() != null) {
                    assertEquals(CompletionReason.WQ_CHANGE, current.completionReason());
    ...
    class EdgeCases {
        ...
        @Test
        @DisplayName("Non-WQ-Change event types should be ignored")
        void nonWQChangeIgnored() {
            engine.processEvent(workingTruck("TT01"));
            engine.processEvent(scheduleWithTTWorkflow(1L, 0, WI_ID_1));
            engine.processEvent(new TimeEvent(now));
            ...
            assertFalse(effects.stream().anyMatch(e ->
                    e instanceof ActionCompleted ac && ac.reason() == CompletionReason.WQ_CHANGE),
                    "Non-WQ-Change should not produce WQ_CHANGE completions");
        }
        ...
        void unknownWorkInstructionIdIgnored() {
            ...
            List<SideEffect> effects = engine.processEvent(wqChangeEvent(9999L));
        ...
        @Test
        @DisplayName("WQ Change for WQ without schedule should be ignored")
        void wqChangeWithoutScheduleIgnored() {
            List<SideEffect> effects = engine.processEvent(wqChangeEvent(WI_ID_1));

            assertTrue(effects.isEmpty() || effects.stream().noneMatch(e ->
                    e instanceof ActionCompleted ac && ac.reason() != null),
                    "WQ Change without schedule should produce no force-completed actions");
        }
    }
